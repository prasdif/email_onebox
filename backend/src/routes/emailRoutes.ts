...
import { searchEmails, updateEmailCategory } from '../config/elasticsearch';
 ...
 */
router.patch('/:messageId/category', async (req: Request, res: Response) => {
  try {
    const { messageId } = req.params;
    ...
    // Update in database
    await query(
      'UPDATE emails SET category = $1, updated_at = CURRENT_TIMESTAMP WHERE message_id = $2',
      [category, messageId]
    );

    // Update in Elasticsearch
    await updateEmailCategory(messageId, category);
