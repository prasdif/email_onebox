...
class RAGService {
    ...
    async generateReplies(emailContext) {
        const text = `${emailContext.subject} ${emailContext.body}`.toLowerCase();
        // Find matching scenario
        let bestMatch = null;
        let maxMatches = 0;
        for (const template of this.replyTemplates) {
            const matches = template.keywords.filter(keyword => text.includes(keyword.toLowerCase())).length;
        ...
        if (bestMatch) {
            ...
            if (bestMatch.scenario.includes('job')) {
                suggestions.push(`Thank you for considering my application. I'm enthusiastic about this opportunity and available for next steps. Calendar: ${this.calendarLink}`);
                suggestions.push(`I appreciate you reaching out. I'd love to learn more about this role. When would be convenient for a discussion?`);
            }
            else if (bestMatch.scenario.includes('meeting')) {
                suggestions.push(`That sounds great! I'm available this week. Please pick a time: ${this.calendarLink}`);
            }
        }
        else {
            // Generic professional replies
            suggestions.push(`Thank you for your email. I'd be happy to discuss this further. Please feel free to schedule a time: ${this.calendarLink}`);
            suggestions.push(`I appreciate you reaching out. Could you provide more details about this opportunity?`);
            suggestions.push(`Thank you for contacting me. I'll review your message and respond within 24 hours.`);
    ...
    async storeEmailEmbedding(emailId, emailData) {
    ...
    async getStoredContext(emailId) {
        const key = `email:${emailId}`;
    ...
    async analyzeEmailSentiment(text) {
